...
test('add task', () => {
  ...
  const newState = taskReducer(initialState, addTaskAC({task: newTask}))
...
test('update task', () =>{
  ...
  const newState = taskReducer(initialState, updateTaskAC({task: newTask}))
...
test('add todolist', () => {
  ...
  const newState = taskReducer(initialState, addTodolistsAC({todolist: newTodolist}))
  expect(newState[newTodolist.id]).toEqual([])
})
test('remove todolist', () => {
  const newState = taskReducer(initialState, removeTodolistAC({idTodolist: '86dcbe6e-611d-424e-b717-e90127370564'}))
